...
function TestPaysofterPromiseSeller() {
  ...
  const getSellerPromiseTestState = useSelector(
    (state) => state.getSellerPromiseTestState
  );
  const { loading, promises, error } = getSellerPromiseTestState;
  ...
  useEffect(() => {
    dispatch(getSellerPromisesTest());
  }, [dispatch]);
      ...
      <Row>
        <Col>
          <h1 className="text-center py-3">
            <i className="fas fa-money-bill-wave"></i> Test Promises (Seller)
          </h1>
          {loading ? (
            <Loader />
          ) : error ? (
...
export default TestPaysofterPromiseSeller;
