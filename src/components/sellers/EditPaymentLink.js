...
import { updatePaymentLink, getPaymentLinkDetail } from "../../redux/actions/paymentActions";
...
function EditPaymentLink({ match }) {
  ...
  useEffect(() => {
    dispatch(getPaymentLinkDetail(match.params.id));
  }, [dispatch, match]);

  const updatePaymentLinkState = useSelector((state) => state.updatePaymentLinkState);
  ...
  const handleEditAd = () => {
    ...
    dispatch(updatePaymentLink(editAdFormData));
  };
